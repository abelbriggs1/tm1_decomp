...
class EnvironmentDirectories:
    ...
    matchings: Path
    # Directory for "expected" objects.
    expected: Path
...
class EnvironmentFiles:
    ...
    final_rom: Path
    # Path to `objdiff` CLI binary.
    objdiff_cli: Path

    # Path to the `permuter_config.toml` created by the build.
    permuter_config: Path
    # Path to the `objdiff.json` config created by the build.
    objdiff_config: Path
    # Path to the `report.json` progress report created by the build.
    ...
    def for_version(
        ...
        return EnvironmentFiles(
            ...
            objdiff_cli=directories.tools / "objdiff/objdiff-cli-linux-x86_64",
            permuter_config=directories.root / "permuter_settings.toml",
            objdiff_config=directories.root / "objdiff.json",
            progress_report=directories.config / "report.json",
        )
...
class EnvironmentToolchain:
    ...
    # Base flags for C preprocessor, ignoring includes.
    cpp_flags: list[str]
    # Base flags for C compilation, ignoring includes.
    c_flags: list[str]
    # Base flags for C++ compilation, ignoring includes.
    cxx_flags: list[str]
    # Base flags for the `maspsx` preprocessor.
    ...
    def get_g_level(self, tu: str) -> str:
        """
        Determine the `-G` level this translation unit should use during C
        compilation.
        """
        # `-G0` default.
    ...
    def for_version(
        ...
        """
        compiler = str(directories.tools / "gcc_2.7.2-psx/cc1")

        return EnvironmentToolchain(
            c_compiler_cmd=compiler,
            cxx_compiler_cmd=compiler,
            maspsx_cmd=f"{sys.executable} {directories.tools}/maspsx/maspsx.py",
...
class Environment:
    ...
    def generate_c_compiler_cmd(self) -> str:
        ...
        input/output parameters.
        """
        return f"{self.toolchain.c_compiler_cmd} {' '.join(self.toolchain.c_flags)}"
    ...
    def generate_cxx_compiler_cmd(self) -> str:
        ...
        input/output parameters.
        """
        return f"{self.toolchain.cxx_compiler_cmd} -c {' '.join(self.toolchain.cxx_flags)}"

    def generate_maspsx_cmd(self, input: str, output: str) -> str:
        """
        Generate the full `maspsx` invocation for this environment.
        """
        flags = [
            *[f"-I{inc}" for inc in self.directories.includes],
