...
def _prepare_rom(env: Environment):
    ...
    if not env.files.disk_rom.is_file():
        ...
        # PSYQ PSX binaries aren't actually ELFs that `objcopy` can read. Just lop off
        # the 0x800 header.
        contents = env.files.disk.read_bytes()
        env.files.disk_rom.write_bytes(contents[0x800:])
